...
function mos_img2cnet(mode, id) {
    ...
    if (mode === "img") {
        const cb = gradioApp()
            .getElementById(`img2img_controlnet_ControlNet-${id}_controlnet_same_img2img_checkbox`)
            .querySelector("input[type='checkbox']");
        cb.checked = true;
        updateInput(cb);
    }

    const m_cb = gradioApp()
        .getElementById(`${mode}2img_controlnet_ControlNet-${id}_controlnet_mask_upload_checkbox`)
        .querySelector("input[type='checkbox']");
    m_cb.checked = true;
    updateInput(m_cb);
    ...
    if (mode === "txt") switch_to_txt2img();
    else switch_to_img2img();
