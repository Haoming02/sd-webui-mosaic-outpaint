...
def mos_ui():
    ...
    with gr.Blocks() as mos_UI:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Row() as stretch_config:
                    stretch_area = gr.Slider(label='Stretch %', minimum=0.25, maximum=0.75, step=0.05, value=0.50)
                    stretch_scale = gr.Slider(label='Stretch Ratio', minimum=1, maximum=5, step=1, value=2)
            ...
            with gr.Column(scale=1):
                ...
                gr.Markdown('<p align="right"><sub>v2.4</sub></p>', elem_id='mos_ver')
        ...
        proc_btn.click(
            process_mask,
            inputs=[input_img, directions, method, stretch_area, stretch_scale, expansion_X, expansion_Y, overlap, steps_S, steps_L],
        ...
        cnet_send_btn.click(
            None, [cnet_mode, cnet_id], None,
            _js='(m, i) => { mos_img2cnet(m, i); }'
        )

        for comp in [input_img, directions, method, stretch_area, stretch_scale, expansion_X, expansion_Y, overlap, steps_S, steps_L, cnet_mode, cnet_id]:
